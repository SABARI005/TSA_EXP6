...
# Load the dataset
file_path = 'data_date.csv'  # Replace with your file path
...
# Select relevant columns ('Date' and 'AQI Value')
commodity_data = country_data[['Date', 'AQI Value']]

# Set 'Date' as the index
commodity_data.set_index('Date', inplace=True)

# Resample the data to monthly averages
monthly_data = commodity_data['AQI Value'].resample('M').mean()
...
# Fit Holt-Winters model (additive trend only, no seasonality)
